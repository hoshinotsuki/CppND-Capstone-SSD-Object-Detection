...
int SSDModel::getClassSize() { return classes.size(); }
...
void SSDModel::objectDetection() {
  ...
  while (true) {
    ...
    std::vector<int> indices =
        detect(current_image, classIds, confidences, boxes);
    ...
    detect_classIds.push(std::move(classIds_out));
...
// The output from the network is set to reference parameters(classIds,
// confidences, boxes). Return: indices which is used to pick up the final
// detection to show
...
std::vector<int> SSDModel::detect(const cv::Mat &image,
  ...
  cv::Mat blob =
      cv::dnn::blobFromImage(image, 1.0, sz, cv::Scalar(), swapRB, false);
  ...
  for (size_t k = 0; k < outs.size(); k++) {
    ...
    for (size_t i = 0; i < outs[k].total(); i += 7) {
      ...
      if (confidence > conf_threshold) {
        ...
        // Add 1 because cv::Rect() defines the boundary as left and top are
        // inclusive,
        ...
        classIds.push_back(classId -
                           1); // classID=0 is background, and we have to start
...
void SSDModel::loadModel() {
  ...
  if (outLayerType != "DetectionOutput")
    CV_Error(cv::Error::StsNotImplemented,
             "Unexpected output layer type: " + outLayerType);
}
