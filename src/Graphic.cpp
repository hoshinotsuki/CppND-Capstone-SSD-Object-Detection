...
Graphic::Graphic(std::string _img_path, int class_num) : image_path(_img_path) {
  ...
  // desired frequency of object detection(every quarter of fps)
  _detect_freq = ((int)_fps) / 4;
  image_width = (int)cap.get(cv::CAP_PROP_FRAME_WIDTH);
  image_height = (int)cap.get(cv::CAP_PROP_FRAME_HEIGHT);
  window_size = resizedSize(cv::Size(image_width, image_height));

  std::cout << "Image file : " << image_path << std::endl;
  std::cout << "- original width = " << image_width << std::endl;
  std::cout << "- original height = " << image_height << std::endl;
  std::cout << "- fps = " << _fps << std::endl;

  cap.release();
}

Graphic::~Graphic() { read_thread.join(); }

void Graphic::thread_for_read() {
  read_thread = std::thread(&Graphic::readImage, this);
}

cv::Size Graphic::getWindowSize() { return window_size; }

void Graphic::setImageQueue(
    std::shared_ptr<MessageQueue<cv::Mat>> _image_queue) {
  image_queue = _image_queue;
}

void Graphic::setDetectionQueue(
    std::shared_ptr<MessageQueue<cv::Mat>> _detect_queue) {
  detect_queue = _detect_queue;
}

float Graphic::getFps() { return _fps; }

int Graphic::getDetectFreq() { return _detect_freq; }

// Draw the result of detection on image(reference parameter)
void Graphic::drawResult(cv::Mat &image, const std::vector<int> &classIds,
  ...
  for (size_t i = 0; i < classIds.size(); i++) {
    ...
    if (boxes[i].width * 1.0 / image_width > 0.5 ||
      ...
      std::cout << "Collision Avoidance for " << classNames[i] << ": width "
                << boxes[i].width << " over image_width " << image_width
                << std::endl;
...
void Graphic::readImage() {
  ...
  if (!cap.isOpened()) {
    CV_Error(cv::Error::StsError,
             "Image file (" + image_path + ") cannot open.");
  }
...
void Graphic::setClassColor(int class_num) {
  std::mt19937 random_engine(
      2019); // Use a fixed seed to get same colors always
  std::uniform_int_distribution<int> distribution(0, 255);

  for (int i = 0; i < class_num; ++i) {
    cv::Scalar color =
        cv::Scalar(distribution(random_engine), distribution(random_engine),
